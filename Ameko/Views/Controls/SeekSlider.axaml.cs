...
public sealed partial class SeekSlider : RangeBase
{
    ...
    public static readonly StyledProperty<bool> IsDirectionReversedProperty =
        Track.IsDirectionReversedProperty.AddOwner<SeekSlider>();
    ...
    public static readonly StyledProperty<double> TickFrequencyProperty = AvaloniaProperty.Register<
        SeekSlider,
        double
    >(nameof(TickFrequency), 0.0);

    /// <summary>
    /// Defines the <see cref="TickPlacement"/> property.
    /// </summary>
    public static readonly StyledProperty<TickPlacement> TickPlacementProperty =
        AvaloniaProperty.Register<SeekSlider, TickPlacement>(nameof(TickPlacement), 0d);

    /// <summary>
    /// Defines the <see cref="Ticks"/> property.
    /// </summary>
    public static readonly StyledProperty<AvaloniaList<double>?> TicksProperty =
        TickBar.TicksProperty.AddOwner<SeekSlider>();
    ...
    static SeekSlider()
    {
        PressedMixin.Attach<SeekSlider>();
        FocusableProperty.OverrideDefaultValue<SeekSlider>(true);
        OrientationProperty.OverrideDefaultValue<SeekSlider>(Orientation.Horizontal);
        Thumb.DragStartedEvent.AddClassHandler<SeekSlider>(
            (x, e) => x.OnThumbDragStarted(e),
            RoutingStrategies.Bubble
        );
        Thumb.DragCompletedEvent.AddClassHandler<SeekSlider>(
            (x, e) => x.OnThumbDragCompleted(e),
            RoutingStrategies.Bubble
        );

        ValueProperty.OverrideMetadata<SeekSlider>(
            new StyledPropertyMetadata<double>(enableDataValidation: true)
        );
        AutomationProperties.ControlTypeOverrideProperty.OverrideDefaultValue<SeekSlider>(
    ...
    /// <summary>
    /// Gets or sets a value that indicates whether the <see cref="SeekSlider"/>
    /// automatically moves the <see cref="Thumb"/> to the closest tick mark.
    /// </summary>
    ...
    /// <summary>
    /// Gets a value indicating whether the <see cref="SeekSlider"/> is currently being dragged.
    /// </summary>
    public bool IsDragging { get; private set; }

    /// <summary>
    /// Gets the <see cref="Track"/> part of the <see cref="SeekSlider"/>.
    /// </summary>
    ...
    protected override void OnApplyTemplate(TemplateAppliedEventArgs e)
    {
        ...
        {
            _decreaseButtonPressDispose = _decreaseButton.AddDisposableHandler(
                PointerPressedEvent,
                TrackPressed,
                RoutingStrategies.Tunnel
            );
            _decreaseButtonReleaseDispose = _decreaseButton.AddDisposableHandler(
        ...
        {
            _increaseButtonSubscription = _increaseButton.AddDisposableHandler(
                PointerPressedEvent,
                TrackPressed,
                RoutingStrategies.Tunnel
            );
            _increaseButtonReleaseDispose = _increaseButton.AddDisposableHandler(
                PointerReleasedEvent,
                TrackReleased,
                RoutingStrategies.Tunnel
            );
        ...
        _pointerMovedDispose = this.AddDisposableHandler(
            PointerMovedEvent,
            TrackMoved,
            RoutingStrategies.Tunnel
        );
    ...
    private void MoveToNextTick(double direction)
    {
        ...
        if (
            Math.Abs(next - value) < Tolerance
            && !(greaterThan && Math.Abs(value - Maximum) < Tolerance) // Stop if searching up is already at Max
            && !(!greaterThan && Math.Abs(value - Minimum) < Tolerance)
        ) // Stop if searching down is already at Min
        {
            var ticks = Ticks;
            ...
            if (ticks is { Count: > 0 })
            {
                foreach (var tick in ticks)
                {
                    ...
                    if (
                        greaterThan
                            && MathUtils.GreaterThan(tick, value)
                            && (
                                MathUtils.LessThan(tick, next) || Math.Abs(next - value) < Tolerance
                            )
                        || !greaterThan
                            && MathUtils.LessThan(tick, value)
                            && (
                                MathUtils.GreaterThan(tick, next)
                                || Math.Abs(next - value) < Tolerance
                            )
                    )
                    {
    ...
    private void MoveToPoint(PointerPoint posOnTrack)
    {
        ...
        var thumbLength =
            (orient ? Track.Thumb?.Bounds.Width ?? 0.0 : Track.Thumb?.Bounds.Height ?? 0.0)
            + double.Epsilon;
        var trackLength = (orient ? Track.Bounds.Width : Track.Bounds.Height) - thumbLength;
        var trackPos = orient ? posOnTrack.Position.X : posOnTrack.Position.Y;
        var logicalPos = MathUtils.Clamp((trackPos - thumbLength * 0.5) / trackLength, 0.0d, 1.0d);
        var invert = orient
            ? IsDirectionReversed
                ? 1
                : 0
    ...
    private double SnapToTick(double value)
    {
        ...
        // Choose the closest value between previous and next. If tied, snap to 'next'.
...
/// <summary>
/// Enum which describes how to position the ticks in a <see cref="SeekSlider"/>.
/// </summary>
public enum TickPlacement
{
    ...
    /// <summary>
    /// Tick marks  will appear above the track for a horizontal <see cref="SeekSlider"/>, or to the left of the track for a vertical <see cref="SeekSlider"/>.
    /// </summary>
    TopLeft,

    /// <summary>
    /// Tick marks will appear below the track for a horizontal <see cref="SeekSlider"/>, or to the right of the track for a vertical <see cref="SeekSlider"/>.
    /// </summary>
    BottomRight,

    /// <summary>
    /// Tick marks appear on both sides of either a horizontal or vertical <see cref="SeekSlider"/>.
    /// </summary>
    Outside,
}
