// SPDX-License-Identifier: MPL-2.0

namespace AssCS.Overrides;

public static class OverrideTags
{
    public const string A = "a";
    public const string A1 = "1a";
    public const string A2 = "2a";
    public const string A3 = "3a";
    public const string A4 = "4a";
    public const string Alpha = "alpha";
    public const string An = "an";
    public const string B = "b";
    public const string Be = "be";
    public const string Blur = "blur";
    public const string Bord = "bord";
    public const string C = "c";
    public const string C1 = "1c";
    public const string C2 = "2c";
    public const string C3 = "3c";
    public const string C4 = "4c";
    public const string Clip = "clip";
    public const string Fad = "fad";
    public const string Fade = "fade";
    public const string FaX = "fax";
    public const string FaY = "fay";
    public const string Fe = "fe";
    public const string Fn = "fn";
    public const string Fr = "fr";
    public const string FrX = "frx";
    public const string FrY = "fry";
    public const string FrZ = "frz";
    public const string Fs = "fs";
    public const string Fsc = "fsc";
    public const string FscX = "fscx";
    public const string FscY = "fscy";
    public const string Fsp = "fsp";
    public const string I = "i";
    public const string IClip = "iclip";
    public const string K = "k";
    public const string Kf = "kf";
    public const string Ko = "ko";
    public const string Kt = "kt";
    public const string KUpper = "K";
    public const string Move = "move";
    public const string Org = "org";
    public const string P = "p";
    public const string Pbo = "pbo";
    public const string Pos = "pos";
    public const string Q = "q";
    public const string R = "r";
    public const string S = "s";
    public const string Shad = "shad";
    public const string T = "t";
    public const string U = "u";
    public const string XBord = "xbord";
    public const string XShad = "xshad";
    public const string YBord = "ybord";
    public const string YShad = "yshad";
}
